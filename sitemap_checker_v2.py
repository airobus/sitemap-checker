...
def send_push_notification_for_domain(new_urls, domain_prefix):
    ...
    if not token:
        # 这个错误只在第一次配置时需要关注，后续在 Actions 中运行时此消息不会显示
        return
...
def process_sitemap(sitemap_url):
    ...
    try:
        response = requests.get(sitemap_url, timeout=30) # 设置30秒超时
        response.raise_for_status()
...
if __name__ == "__main__":
    ...
    if not os.path.exists(DOMAINS_FILE):
        ...
        if not urls_to_process:
            ...
            # 使用线程池并发执行 process_sitemap 函数
            with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
                # map会按照urls_to_process的顺序，将每个url传递给process_sitemap函数
